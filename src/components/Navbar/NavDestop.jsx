      ...
      <nav className="absolute left-0 ">
         ...
         >
            <div className='relative' >
               ...
               <ul className={`space-y-4 ${isHovered ? 'px-4 py-2' : 'px-3'} pt-4`}>
                  {navItems.map((item, i) => (
                     <li
                        key={i} className={`flex items-center text-gray-500 space-x-4 cursor-pointer transition-all duration-300 ease-in-out ${isHovered ? `px-2 py-1 hover:bg-gray-200 hover:pl-3 rounded-md ${isActive === item.path ? 'px-2 py-1 bg-gray-200 rounded-md' : ''}` : ''}`}
